...
def test_rates_from_valid(client: TestClient):
    ...
    # Get data from API
    response = client.get(
        f"/rates/from/EURUSD/{mt5.TIMEFRAME_D1}/{now.isoformat()}/{count}"
    )
    ...
    for i, api_rate in enumerate(json_data):
        ...
        assert api_rate["tick_volume"] == int(mt5_rate["tick_volume"]), (
            f"Rate {i}: tick_volume mismatch"
        )
        assert api_rate["spread"] == int(mt5_rate["spread"]), (
            f"Rate {i}: spread mismatch"
        )
        assert api_rate["real_volume"] == int(mt5_rate["real_volume"]), (
            f"Rate {i}: real_volume mismatch"
        )
...
def test_rates_from_pos_valid(client: TestClient):
    ...
    # Get data from API
    response = client.get(
        f"/rates/from-pos/EURUSD/{mt5.TIMEFRAME_D1}/{start_pos}/{count}"
    )
...
def test_rates_range_valid(client: TestClient):
    ...
    # Verify response structure
    assert isinstance(json_data, list), "Response should be a list"
    assert len(json_data) == len(mt5_rates), (
        f"Expected {len(mt5_rates)} rates, got {len(json_data)}"
    )
...
def test_rates_from_invalid_symbol(client: TestClient):
    ...
    now = datetime.now()
    response = client.get(
        f"/rates/from/INVALIDSYMBOL/{mt5.TIMEFRAME_D1}/{now.isoformat()}/20"
    )
