...
def test_orders_get_by_symbol(client: TestClient):
    ...
    if mt5_orders is None:
        ...
        # Verify response structure
        assert isinstance(json_data, list), "Response should be a list"
        assert len(json_data) == len(mt5_orders), f"Expected {len(mt5_orders)} orders for {symbol}, got {len(json_data)}"
        
        # Verify all orders are for the specified symbol
        for order in json_data:
            assert order.get("symbol") == symbol or order.get("symbol") is None, f"Order should be for symbol {symbol}"
...
def test_orders_get_by_ticket(client: TestClient, request):
    ...
    if isinstance(json_data, list):
        ...
        if len(json_data) == 1:
            assert json_data[0]["ticket"] == ticket, "Returned order ticket should match"
    else:
...
def test_orders_calc_margin_valid(client: TestClient):
    ...
    assert abs(api_margin - mt5_margin) < 0.01, f"Expected {mt5_margin}, got {api_margin}"
...
def test_orders_calc_profit_valid(client: TestClient):
    ...
    # Compare with MT5 (allowing for small floating point differences)
    assert abs(api_profit - mt5_profit) < 0.01, f"Expected {mt5_profit}, got {api_profit}"
...
def test_orders_calc_margin_zero_volume(client: TestClient):
    ...
    assert isinstance(json_data, (int, float, dict)), "Response should be numeric or error dict"
...
def test_orders_different_order_types(client: TestClient):
    ...
    for order_type in order_types:
        ...
        assert response.status_code == 200, f"Status code should be 200 for order type {order_type}"
        json_data = response.json()
        
        # Should return numeric value or error dict
        assert isinstance(json_data, (int, float, dict)), f"Response should be numeric or error dict for order type {order_type}"
