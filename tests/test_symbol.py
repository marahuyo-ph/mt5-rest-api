...
def test_symbols_get_all(client: TestClient):
    ...
    assert len(json_data) == len(mt5_symbols), (
        f"Expected {len(mt5_symbols)} symbols, got {len(json_data)}"
    )
    ...
    for i, api_symbol in enumerate(json_data):
        ...
        assert api_symbol["name"] == mt5_symbol._asdict()["name"], (
            f"Symbol {i}: name mismatch"
        )
...
def test_symbols_get_with_group(client: TestClient):
    ...
    assert isinstance(json_data, list), "Response should be a list"
    assert len(json_data) == len(mt5_symbols), (
        f"Expected {len(mt5_symbols)} symbols in group '{group}', got {len(json_data)}"
    )
...
def test_multiple_symbols_info(client: TestClient):
    ...
    for symbol in symbols:
        ...
        assert isinstance(json_data, dict), (
            f"Response for {symbol} should be a dictionary"
        )
