...
def test_ticks_from_valid(client: TestClient):
    ...
    assert len(json_data) == len(mt5_ticks), (
        f"Expected {len(mt5_ticks)} ticks, got {len(json_data)}"
    )
    ...
    for i, api_tick in enumerate(json_data):
        ...
        assert api_tick["volume"] == int(mt5_tick["volume"]), (
            f"Tick {i}: volume mismatch"
        )
        assert api_tick["time_msc"] == int(mt5_tick["time_msc"]), (
            f"Tick {i}: time_msc mismatch"
        )
        assert api_tick["flags"] == int(mt5_tick["flags"]), f"Tick {i}: flags mismatch"
        assert api_tick["volume_real"] == mt5_tick["volume_real"], (
            f"Tick {i}: volume_real mismatch"
        )
...
def test_ticks_from_invalid_symbol(client: TestClient):
    ...
    flags = mt5.COPY_TICKS_ALL
    response = client.get(
        f"/ticks/from/INVALIDSYMBOL/{now.isoformat()}/100?flags={flags}"
    )
