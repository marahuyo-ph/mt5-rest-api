...
def test_positions_get_all(client: TestClient):
    ...
    if mt5_positions is None or len(mt5_positions) == 0:
        ...
        assert isinstance(json_data, dict), "Empty response should be a dictionary (error response)"
        # MT5 error response has code and message fields
        assert "code" in json_data or "retcode" in json_data, "Error response should have code/retcode field"
    else:
        # Verify response structure
        assert isinstance(json_data, list), "Response should be a list"
        assert len(json_data) == len(mt5_positions), f"Expected {len(mt5_positions)} positions, got {len(json_data)}"
...
def test_positions_get_by_symbol(client: TestClient):
    ...
    if mt5_positions is None or len(mt5_positions) == 0:
        ...
        for position in json_data:
            assert position.get("symbol") == symbol, f"Position should be for symbol {symbol}"
...
def test_positions_get_by_ticket(client: TestClient, request):
    ...
    if isinstance(json_data, list):
        ...
        if len(json_data) == 1:
            assert json_data[0]["ticket"] == ticket, "Returned position ticket should match"
    else:
...
def test_positions_structure(client: TestClient, request):
    ...
    if isinstance(json_data, list) and len(json_data) > 0:
        ...
        assert api_position["price_open"] == mt5_dict["price_open"], "Position price_open mismatch"
...
def test_positions_numeric_fields(client: TestClient, request):
    ...
    if isinstance(json_data, list) and len(json_data) > 0:
        ...
        numeric_fields = [
            "ticket", "type", "magic", "time_update", "time_update_msc",
            "volume", "price_open", "price_current", "swap", "profit"
        ]
...
def test_positions_buy_sell_types(client: TestClient, request):
    ...
    if isinstance(json_data, list) and len(json_data) > 0:
        ...
        for position in json_data:
            ...
            assert position_type in valid_types, f"Position type {position_type} is not valid"
...
def test_positions_consistency(client: TestClient):
    ...
    if isinstance(positions, list) and len(positions) > 0:
        assert len(positions) == total, f"Position count mismatch: total={total}, list={len(positions)}"
