...
class Position(BaseModel):
    ...
    time_update_msc: int = Field(
        default=0, description="Position change time in milliseconds since 01.01.1970"
    )
    ...
    identifier: int = Field(
        default=0, description="Position identifier (unique throughout lifetime)"
    )
    reason: Optional[PositionReason] = Field(
        default=None, description="Position opening reason"
    )
    ...
    price_current: float = Field(
        default=0.0, description="Current price of position symbol"
    )
